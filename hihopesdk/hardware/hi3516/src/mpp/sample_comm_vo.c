...
//{	  0 ,  1,  1,1280,	24,   8,	800,	43,  99,	 1, 	 1, 	1,		1, 24,   4,  0,   0,  0},	 /* For MIPI DSI Tx 800 1280 at 60 Hz */ 
void init_pstAttr(VO_PUB_ATTR_S* pstPubAttr)
{
	pstPubAttr->stSyncInfo.bSynm=HI_TRUE;
	pstPubAttr->stSyncInfo.bIop=HI_TRUE;
	...
	pstPubAttr->stSyncInfo.u16Hbb=43;
	pstPubAttr->stSyncInfo.u16Hfb=99;
	...
	pstPubAttr->stSyncInfo.u16Hpw=24;
	pstPubAttr->stSyncInfo.u16Vpw=4;
	pstPubAttr->stSyncInfo.bIdv=HI_FALSE;
	pstPubAttr->stSyncInfo.bIhs=HI_FALSE;
	pstPubAttr->stSyncInfo.bIvs=HI_FALSE;
...
combo_dev_cfg_t CMIPI_TX_800X1280_60_CONFIG =
{
	.devno = 0,
	.lane_id = {0, 1, 2, 3},
	.output_mode = OUTPUT_MODE_DSI_VIDEO,
	...
	.sync_info = {
		.vid_pkt_size	  = 800, // hact
		.vid_hsa_pixels   = 20,  // hsa
		.vid_hbp_pixels   = 20,  // hbp
		.vid_hline_pixels = 872, // hact + hsa + hbp + hfp
		.vid_vsa_lines	  = 4,	 // vsa
		.vid_vbp_lines	  = 6,	// vbp
		.vid_vfp_lines	  = 8,	 // vfp
		.vid_active_lines = 1280,// vact
...
HI_S32 COMM_VO_GetPubAttr(VO_PUB_ATTR_S *pstVoPubAttr)
{
  int ret; // r3@2
  VO_PUB_ATTR_S *pstVoPubAttra; // [sp+18h] [bp-1Ch]@1
  combo_dev_cfg_t *pstComboDevCfg; // [sp+24h] [bp-10h]@5

  pstVoPubAttra = pstVoPubAttr;
  if (pstVoPubAttr )
  {
	memset(pstVoPubAttr, 0, sizeof(VO_PUB_ATTR_S));
	pstVoPubAttra->enIntfType = 0x4000;
	pstVoPubAttra->enIntfSync = VO_OUTPUT_USER;
	pstVoPubAttra->u32BgColor = 0;

	pstComboDevCfg = &CMIPI_TX_800X1280_60_CONFIG;
	
	pstVoPubAttra->stSyncInfo.bSynm = 1;
	pstVoPubAttra->stSyncInfo.bIop = 1;
	pstVoPubAttra->stSyncInfo.u8Intfb = 0;
	pstVoPubAttra->stSyncInfo.u16Hact = pstComboDevCfg->sync_info.vid_pkt_size;
	pstVoPubAttra->stSyncInfo.u16Hbb = pstComboDevCfg->sync_info.vid_hsa_pixels
									 + pstComboDevCfg->sync_info.vid_hbp_pixels;
	pstVoPubAttra->stSyncInfo.u16Hfb = pstComboDevCfg->sync_info.vid_hline_pixels
									 - pstVoPubAttra->stSyncInfo.u16Hact
									 - pstVoPubAttra->stSyncInfo.u16Hbb;
	pstVoPubAttra->stSyncInfo.u16Hmid = 0;
	pstVoPubAttra->stSyncInfo.u16Vact = pstComboDevCfg->sync_info.vid_active_lines;
	pstVoPubAttra->stSyncInfo.u16Vbb = pstComboDevCfg->sync_info.vid_vsa_lines + pstComboDevCfg->sync_info.vid_vbp_lines;
	pstVoPubAttra->stSyncInfo.u16Vfb = pstComboDevCfg->sync_info.vid_vfp_lines;
	pstVoPubAttra->stSyncInfo.u16Bvact = 0;
	pstVoPubAttra->stSyncInfo.u16Bvbb = 0;
	pstVoPubAttra->stSyncInfo.u16Bvfb = 0;
	pstVoPubAttra->stSyncInfo.u16Hpw = pstComboDevCfg->sync_info.vid_hsa_pixels;
	pstVoPubAttra->stSyncInfo.u16Vpw = pstComboDevCfg->sync_info.vid_vsa_lines;
	pstVoPubAttra->stSyncInfo.bIdv = 0;
	pstVoPubAttra->stSyncInfo.bIhs = 0;
	pstVoPubAttra->stSyncInfo.bIvs = 0;
	printf(
	  "u16Hact:%d, u16Hbb:%d, u16Hfb:%d, u16Vact:%d, u16Vbb:%d, u16Vfb:%d, u16Hpw:%d, u16Vpw:%d.\n",
	  pstVoPubAttra->stSyncInfo.u16Hact,
	  pstVoPubAttra->stSyncInfo.u16Hbb,
	  pstVoPubAttra->stSyncInfo.u16Hfb,
	  pstVoPubAttra->stSyncInfo.u16Vact,
	  pstVoPubAttra->stSyncInfo.u16Vbb,
	  pstVoPubAttra->stSyncInfo.u16Vfb,
	  pstVoPubAttra->stSyncInfo.u16Hpw,
	  pstVoPubAttra->stSyncInfo.u16Vpw);
	ret = 0;
  }
  else
  {
...
HI_S32 SAMPLE_COMM_VO_StartVO(SAMPLE_VO_CONFIG_S *pstVoConfig)
{
    ...
    *********************************/
	...
	COMM_VO_GetPubAttr(&stVoPubAttr);

	printf("stVoPubAttr.enIntfType %d stVoPubAttr.enIntfSync %d \n",stVoPubAttr.enIntfType,stVoPubAttr.enIntfSync);
...
stLayerAttr.stDispRect.s32X =0;
stLayerAttr.stDispRect.s32Y =0;
stLayerAttr.stDispRect.u32Width =800;
stLayerAttr.stDispRect.u32Height =1280;
stLayerAttr.stImageSize.u32Width = stLayerAttr.stDispRect.u32Width;
stLayerAttr.stImageSize.u32Height = stLayerAttr.stDispRect.u32Height;
stLayerAttr.enPixFormat = 26;
stLayerAttr.bClusterMode = 0;
stLayerAttr.bDoubleFrame = 0;
stLayerAttr.u32DispFrmRt = 60;
stLayerAttr.enDstDynamicRange = 0;
#endif
...
static void SAMPLE_PRIVATE_VO_InitMipiTxScreen(VO_INTF_SYNC_E enVoIntfSync, HI_S32 fd)
{
    ...
    {
        SAMPLE_PRT("%s,%d,Init 720p as 800x1280 screen.\n",__FUNCTION__,__LINE__);
        // init screen for 720x1280_60.
        SAMPLE_PRIVATE_VO_InitScreen720x1280(fd);
    }
    else
...
void SAMPLE_COMM_VO_StartMipiTx(VO_INTF_SYNC_E enVoIntfSync)
{
    ...
    switch(enVoIntfSync)
    {
        ...
        case VO_OUTPUT_USER:
            pstMipiTxConfig = &MIPI_TX_800X1280_60_CONFIG;	
			printf("get MIPI_TX_800X1280_60_CONFIG \n");
            break;
        case VO_OUTPUT_1080x1920_60:
