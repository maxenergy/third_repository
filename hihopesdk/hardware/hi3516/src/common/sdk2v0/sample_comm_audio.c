...
int hisi_audio_open()
{
		...
		HI_MPI_AO_GetVolume(AoDev,&s32VolumeDb);
		s32VolumeDb = 2;
		s32Ret = HI_MPI_AO_SetVolume(AoDev, s32VolumeDb);
...
int hisi_audio_write(char* buf,int size)
{
    AUDIO_STREAM_S stAudioStream;
...
#if 1
int hisi_audio_read(char* buf,int size)
{
    AUDIO_STREAM_S stAudioStream;
	HI_S32 s32Ret;
	stAudioStream.u32Len = 0;
	/* get stream from aenc chn */
		s32Ret = HI_MPI_AENC_GetStream(0, &stAudioStream, 20);
		if (HI_SUCCESS != s32Ret)
		{
			stAudioStream.u32Len = 0;
		...
		if(stAudioStream.u32Len > 0){
			if(stAudioStream.u32Len > size*2){				
				printf("error size! %d >  %d\n",stAudioStream.u32Len,size);
				stAudioStream.u32Len = size*2;
			}
			memcpy(buf,stAudioStream.pStream,stAudioStream.u32Len);
		}			
        s32Ret = HI_MPI_AENC_ReleaseStream(0, &stAudioStream);
	return stAudioStream.u32Len;
}
#else
int hisi_audio_read(char* buf,int size)
{
	...
	/* get stream from aenc chn */
	while(time_out<10){
		s32Ret = HI_MPI_AENC_GetStream(0, &stAudioStream, 40);
		if (HI_SUCCESS != s32Ret )
		{
			//usleep(5000);
			time_out++;
