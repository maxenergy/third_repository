    ...
    SAMPLE_SVP_NNIE_YOLOV2_BBOX_S *pstBbox2)
{
	...
	//if(f64InterArea > f64Box1Area*0.8 || f64InterArea > f64Box2Area * 0.8)
	//return 0.8;
        ...
        if(pstBbox[i].u32Mask == 0 )
        {
            ...
            for(j= i+1;j< u32BboxNum; j++)
            {
                if( pstBbox[j].u32Mask == 0 )
                {
				    ...
				    //printf("clss id %d -- %d iou %f \n",pstBbox[i].u32ClassIdx,pstBbox[j].u32ClassIdx,f64Iou);
    ...
    HI_S32 *ps32ClassRoiNum)
{
	...
	static HI_S32 SVP_NNIE_Yolov3_tiny_GetResult(HI_S32 **pps32InputData,HI_U32 au32GridNumWidth[],
		HI_U32 au32GridNumHeight[],HI_U32 au32Stride[],HI_U32 u32EachGridBbox,HI_U32 u32ClassNum,HI_U32 u32SrcWidth,
		HI_U32 u32SrcHeight,HI_U32 u32MaxRoiNum,HI_U32 u32NmsThresh,HI_U32 u32ConfThresh,
		HI_FLOAT af32Bias[SAMPLE_SVP_NNIE_YOLOV3_TINY_REPORT_BLOB_NUM][SAMPLE_SVP_NNIE_YOLOV3_TINY_EACH_GRID_BIAS_NUM],
		HI_S32* ps32TmpBuf,HI_S32 *ps32DstScore, HI_S32 *ps32DstRoi, HI_S32 *ps32ClassRoiNum)
	{
		...
		for(i = 0; i < SAMPLE_SVP_NNIE_YOLOV3_TINY_REPORT_BLOB_NUM; i++)
		{
		...
		for(i = 0; i < SAMPLE_SVP_NNIE_YOLOV3_TINY_REPORT_BLOB_NUM; i++)
		{
			...
			for(j = 0; j < au32GridNumWidth[i]*au32GridNumHeight[i]; j++)
			{
				...
				for (k = 0; k < u32EachGridBbox; k++)
				{
					...
					{
				//	  	printf("f32ObjScore %f f32MaxScore %f s32ClassScore%d u32ConfThresh %d u32MaxValueIndex %d \n",f32ObjScore,f32MaxScore,s32ClassScore,u32ConfThresh,u32MaxValueIndex);
					//decode bbox
...
{
	HI_U32 i = 0;
	HI_S32 *aps32InputBlob[SAMPLE_SVP_NNIE_YOLOV3_TINY_REPORT_BLOB_NUM] = {0};
	HI_U32 au32Stride[SAMPLE_SVP_NNIE_YOLOV3_TINY_REPORT_BLOB_NUM] = {0};
	...
	return SVP_NNIE_Yolov3_tiny_GetResult(aps32InputBlob,pstSoftwareParam->au32GridNumWidth,
		pstSoftwareParam->au32GridNumHeight,au32Stride,pstSoftwareParam->u32BboxNumEachGrid,
		pstSoftwareParam->u32ClassNum,pstSoftwareParam->u32OriImWidth,
		pstSoftwareParam->u32OriImWidth,pstSoftwareParam->u32MaxRoiNum,pstSoftwareParam->u32NmsThresh,
		pstSoftwareParam->u32ConfThresh,pstSoftwareParam->af32Bias,
