...
#define  MAX_RET_COUNT  100

#define NET_CLASS_NUM 1
...
void opencl_deinit();
unsigned int get_yolo_reslut(int **ret_buf,unsigned int *len,unsigned int *stride,float *af32Bias,unsigned int grid_num,SAMPLE_SVP_NNIE_YOLOV2_BBOX_S *out, int);
#endif
using namespace std;
...
static HI_S32 Hihope_Yolov3_GetResult_L(HI_S32 **pps32InputData,HI_U32 au32GridNumWidth[],
        ...
        HI_S32* ps32TmpBuf,HI_S32 *ps32DstScore, HI_S32 *ps32DstRoi, HI_S32 *ps32ClassRoiNum,HI_S32 Blob_num)
    {
        ...
        u32BboxNum = get_yolo_reslut(pps32InputData,au32GridNumWidth,au32Stride,(float *)af32Bias,Blob_num,pstBbox,u32ClassNum);
...
HI_S32 SAMPLE_SVP_NNIE_Detection_Result_NEW(SVP_BLOB_S *pstDstScore,
	SVP_BLOB_S *pstDstRoi, SVP_BLOB_S *pstClassRoiNum, HI_FLOAT f32PrintResultThresh,DETECT_RET_BUF * ret_buf)
{
	HI_U32 i = 0, j = 0;
  ...
  while(1)
  {
      if (nnie_buf_state[ID] == BUF_EMPTY) {
        ...
        if(nnie_buf_state[ID] == BUF_FILLED)
        {
            ...
            #endif
                (void)SAMPLE_SVP_NNIE_Detection_Result_NEW(&s_stYolov3SoftwareParam[ID].stDstScore,
...
void net_config(SAMPLE_SVP_NNIE_YOLOV3_SOFTWARE_PARAM_S *pstSoftWareParam)
{
    pstSoftWareParam->u32BboxNumEachGrid = 3;
    pstSoftWareParam->u32ClassNum = NET_CLASS_NUM;
    ...
    pstSoftWareParam->u32NmsThresh = (HI_U32)(0.3f*4096);
    pstSoftWareParam->u32ConfThresh = (HI_U32)(0.5f*4096);
...
net_config(&s_stYolov3SoftwareParam[0]);
net_config(&s_stYolov3SoftwareParam[1]);
#if USE_YOLO_OR_TINYYOLO
