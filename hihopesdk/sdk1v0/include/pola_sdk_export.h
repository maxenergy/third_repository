...
//POLA_SDK ERROR_CODE detect(const char *image_buf, int width, int height, ImageType type, Pola_Object **obj, int *length);
 POLA_SDK ERROR_CODE face_detect(ImageType type,const char *image, int width, int height,Pola_Object **handle, int *length);
...
POLA_SDK ERROR_CODE getPose(Pola_Object handle, Pola_Pose *poseblur_result);

POLA_SDK ERROR_CODE face_track(Pola_Object *handle, int length, int *track_id);
